...
class Navigation(commands.Cog):
	...
	async def skip(self, ctx: Interaction):
		...
		if voice is not None:
			...
			await ctx.followup.send(embed=Embed(title="Skipped :next_track:", color=bot.color))
			return

		await ctx.followup.send(embed=Embed(title="I'm not playing anything", color=bot.color))
	...
	async def pause_(self, ctx: Interaction):
		...
		if voice.is_playing():
			voice.pause()
			await ctx.followup.send(embed=Embed(title="Paused :pause_button:", color=bot.color))
			return
	...
	async def resume_(self, ctx: Interaction):
		...
		if voice.is_paused():
			voice.resume()
			await ctx.followup.send(embed=Embed(title="Resumed :arrow_forward:", color=bot.color))
			return

		await ctx.followup.send(embed=Embed(title="Playing is not paused", color=bot.color))
	...
	async def stop_(self, ctx: Interaction):
		...
		await ctx.followup.send(embed=Embed(title="Stopped :stop_button:", color=bot.color))
	...
	async def leave_(self, ctx: Interaction):
		...
		if voice is not None:
			...
			await ctx.followup.send(embed=Embed(title="Disconnected!", color=bot.color), ephemeral=True)
			return

		await ctx.followup.send(embed=Embed(title="I'm not connected to a voice channel", color=bot.color), ephemeral=True)
	...
	async def clear_all(self, ctx: Interaction):
		...
		if bot.music_queue[ctx.guild.id]:
			bot.music_queue[ctx.guild.id] = []
		await ctx.followup.send(embed=Embed(title="Queue cleared! :broom:", color=bot.color))
