...
class Seek(commands.Cog):
	...
	@slash_command(name='seek', description='Seek to a specific time in the song.', guild_ids=main.bot.guild_ids)
	# async function for command
	async def seek(self, ctx, time):
		...
		if len(main.bot.playing[ctx.guild.id]) != 0:
			...
			if voice_channel is None:
				...
				voice.play(FFmpegPCMAudio(before_options=f'-ss {time} -reconnect 1 '
