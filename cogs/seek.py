...
class Seek(commands.Cog):
	...
	async def seek(self, ctx, time):
		...
		if voice == "" or voice is None:
			voice = await self.bot.worp.playing[ctx.guild.id][1].connect()

		elif voice.channel != self.bot.worp.playing[ctx.guild.id][1]:
			await voice.move_to(self.bot.worp.playing[ctx.guild.id][1])
		...
		self.bot.worp.playing[ctx.guild.id][-1] = dt.datetime.utcnow() - dt.timedelta(seconds=int(time))
		# send a message saying the bot is now playing the song
		await ctx.followup.send(
			content=f'Now playing {self.bot.worp.playing[ctx.guild.id][0].title} from {formatted_time} seconds.')
